"""
Photo Curator (Claude Sonnet Edition) - AI-powered photo organizer
Groups photos by event, names them via Claude Sonnet Vision, picks the best shot.
"""
...
def analyze_group_with_claude(group):
    """
    Send thumbnails of a photo group to Claude Sonnet and get back:
    ...
    """
    print(f"  Sending {len(group)} photo(s) to Claude Sonnet for analysis...")

    # Build the content blocks: text prompt + images
    content = []
    ...
    # Add each thumbnail as a base64 image
    for i, record in enumerate(group):
        ...
        content.append({"type": "text", "text": f"(Photo {i + 1}: {record['path'].name})"})
    ...
    try:
        ...
        print(f"  WARNING: Claude Sonnet returned invalid JSON. Using defaults.")
        ...
        print(f"  ERROR calling Claude Sonnet: {e}")
        return {
            "event_name": "Unknown Event",
            "scores": [50] * len(group),
            "best_index": 0,
...
def main():
    print("=" * 60)
    print("  PHOTO CURATOR (Claude Sonnet)")
    ...
    # 4. Analyze each group with Claude Sonnet
    print("\nAnalyzing photos with Claude Sonnet AI...")
    event_results = []

    for i, group in enumerate(groups):
        print(f"\n--- Event group {i + 1}/{len(groups)} ({len(group)} photos) ---")
        analysis = analyze_group_with_claude(group)
    ...
    # Create output directory if it doesn't exist (do NOT clear existing folders)
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    summary_lines = []
    summary_lines.append("# Photo Curator Summary (Claude Sonnet)\n")
    ...
    for i, result in enumerate(event_results):
        ...
        # Build ranked list: pair each photo with its score and original index
        ranked = sorted(
            enumerate(group),
        ...
        for rank, (j, record) in enumerate(ranked, start=1):
            src = record["path"]
            ...
            best_marker = " **BEST**" if j == best_idx else ""
            summary_lines.append(f"| {rank} | {src.name} | {score}{best_marker} |")
    ...
    # Write summary
    summary_lines.append("---\n")
    summary_lines.append(f"*Generated by Photo Curator using Claude Sonnet*\n")
    summary_path = OUTPUT_DIR / "summary.md"
    summary_path.write_text("\n".join(summary_lines), encoding="utf-8")

    # Done
    print("\n" + "=" * 60)
    print("  DONE! (Claude Sonnet)")
