"""
Photo Curator - AI-powered photo organizer
Groups photos by event, names them via Gemini Vision, picks the best shot.
"""
...
def pil_to_bytes(img, fmt="JPEG"):
    """Convert a PIL Image to bytes for the Gemini API."""
    buf = BytesIO()
...
def analyze_group_with_gemini(group):
    """
    Send thumbnails of a photo group to Gemini and get back:
    ...
    """
    print(f"  Sending {len(group)} photo(s) to Gemini for analysis...")

    # Build the content parts: text prompt + images
    parts = []
    ...
    # Add each thumbnail as an image
    for i, record in enumerate(group):
        ...
        parts.append(f"(Photo {i + 1}: {record['path'].name})")
    ...
    try:
        ...
        print(f"  WARNING: Gemini returned invalid JSON. Using defaults.")
...
def main():
    ...
    # 4. Analyze each group with Gemini
    print("\nAnalyzing photos with Gemini AI...")
    event_results = []

    for i, group in enumerate(groups):
        print(f"\n--- Event group {i + 1}/{len(groups)} ({len(group)} photos) ---")
        analysis = analyze_group_with_gemini(group)
    ...
    # Clear previous output (but don't delete the output dir itself)
    ...
    summary_lines = []
    summary_lines.append("# Photo Curator Summary\n")
    ...
    for i, result in enumerate(event_results):
        ...
        summary_lines.append("| Photo | Score |")
        summary_lines.append("|-------|-------|")
        ...
        for j, record in enumerate(group):
            ...
            summary_lines.append(f"| {src.name} | {score}{marker} |")
    ...
    # Write summary
    summary_lines.append("---\n")
    summary_lines.append(f"*Generated by Photo Curator using Gemini 2.0 Flash*\n")
